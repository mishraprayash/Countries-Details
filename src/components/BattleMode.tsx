...
export default function BattleMode({ countries }: BattleModeProps) {
  ...
  if (!countryA || !countryB) {
    return <div className="p-12 text-center text-zinc-500 animate-pulse">Loading Battle Arena...</div>;
  }

  return (
    <div className="rounded-3xl border border-white/10 bg-zinc-950 overflow-hidden shadow-2xl relative">
      {/* Score Header */}
      <div className="absolute top-0 inset-x-0 p-6 flex justify-between items-center z-20 bg-gradient-to-b from-zinc-950/80 to-transparent pointer-events-none">
        <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-zinc-900/80 backdrop-blur border border-white/10">
          <Award className="h-5 w-5 text-amber-400" />
          <span className="font-bold text-white">Score: {score}</span>
        </div>
        <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-zinc-900/80 backdrop-blur border border-white/10">
          <span className="text-sm font-medium text-zinc-400">High Score:</span>
      ...
      <div className="flex flex-col md:flex-row h-[600px]">
        ...
        {/* VS Divider */}
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-30 hidden md:flex items-center justify-center w-16 h-16 rounded-full bg-zinc-900 border-4 border-zinc-950 text-white font-black text-xl shadow-2xl">
          VS
        </div>

        {/* Country B */}
        <div className="flex-1 relative flex flex-col items-center justify-center p-8 bg-zinc-900/50">
            ...
            {gameState === "playing" ? (
              <div className="space-y-4">
                <button
                  ...
                  className="w-full py-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition-all group flex items-center justify-center gap-3"
                >
                  <TrendingUp className="h-6 w-6 text-emerald-400 group-hover:-translate-y-1 transition-transform" />
                  <span className="text-xl font-bold text-white">Higher</span>
                </button>
                <button
                ...
                >
                  ...
                  <span className="text-xl font-bold text-white">Lower</span>
                </button>
                <p className="text-xs font-medium text-zinc-500 pt-2">than {countryA.name.common}</p>
              </div>
            ) : (
              <div className="animate-in zoom-in duration-300">
                ...
                {gameState === "revealed" && (
                  <div className="inline-flex items-center gap-2 text-emerald-400 font-bold bg-emerald-400/10 px-4 py-2 rounded-full">
                    <Sparkles className="h-5 w-5" /> Correct! Next round...
                  </div>
                )}
                {gameState === "gameover" && (
                  <div className="mt-8 space-y-6">
                    <div className="text-red-400 font-bold text-xl uppercase tracking-widest">Game Over</div>
                    <button
                      onClick={restartGame}
                      className="mx-auto flex items-center gap-2 px-8 py-4 rounded-xl bg-white text-black font-bold hover:bg-zinc-200 transition-all active:scale-95 shadow-xl"
                    >
